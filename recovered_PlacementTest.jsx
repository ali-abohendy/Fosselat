...
export default function PlacementTest() {
      ...
      const res = await fetch(`${API}/placement/send-results`, {
        ...
        body: JSON.stringify({
          ...
          results: {
            ...
            highestMasteredId: resultData.highestMasteredId,
  ...
  const renderAudience = () => (
    <div className="pt-screen">
      <div className="pt-card" style={{ maxWidth: 500 }}>
        <p className="pt-step-label">Step 1</p>
        <h2 className="pt-card-title">Who is taking the test?</h2>
        <div className="pt-choices">
          <button className="pt-choice-card" onClick={() => handleAudienceSelect('kids')}>
            <span className="pt-choice-emoji">👧</span>
            <span>Kids <small>(Ages 5–12)</small></span>
          </button>
          <button className="pt-choice-card" onClick={() => handleAudienceSelect('adults')}>
            <span className="pt-choice-emoji">🧑</span>
            <span>Adults <small>(Teens & Adults)</small></span>
          </button>
        </div>
      </div>
    </div>
  ...
  const renderTrack = () => (
    <div className="pt-screen">
      <div className="pt-card" style={{ maxWidth: 540 }}>
        <p className="pt-step-label">Step 2</p>
        <h2 className="pt-card-title">Choose a subject track</h2>
        <div className="pt-choices">
          {data?.tracks.map(t => (
            <button key={t.id} className="pt-choice-card" onClick={() => handleTrackSelect(t)}
              style={{ '--card-accent': ACCENTS[t.id] || 'var(--color-gold)' }}>
              <span className="pt-track-initial">{INITIALS[t.id] || t.id[0].toUpperCase()}</span>
              <span>{t.label}</span>
            </button>
          ))}
        </div>
  ...
  const renderProgram = () => (
    <div className="pt-screen">
      <div className="pt-card" style={{ maxWidth: 540 }}>
        <p className="pt-step-label">Step 3</p>
        <h2 className="pt-card-title">Choose a program — {track?.label}</h2>
        <div className="pt-program-list">
          {track?.programs.map(p => (
            <button key={p.id} className="pt-choice-row" onClick={() => handleProgramSelect(p)}>
              <span className="pt-choice-row-label">{p.label}</span>
